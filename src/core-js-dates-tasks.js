...
function getNextFriday(date) {
  ...
  const daysNextFriday = (5 - day + 7) % 7 || 7;
  const nextFriday = new Date(date);
  nextFriday.setDate(date.getDate() + daysNextFriday);
  return nextFriday;
}
...
function isDateInPeriod(date, period) {
  ...
  const tempStartPeriod = new Date(period.start).getTime();
  const tempEndPeriod = new Date(period.end).getTime();

  return tempDate >= tempStartPeriod && tempDate <= tempEndPeriod;
}
...
function formatDate(date) {
  ...
  return `${dateFormat.format(dateObj)}, ${timeFormat.format(dateObj)}`;
}
